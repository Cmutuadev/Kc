...
# ---------------------------
# Functions to handle last claim time for daily credits
# ---------------------------

async def get_last_claim_time(user_id: str) -> float:
    """
    Fetch the user's last claim time as a Unix timestamp (float).
    Returns None if the user has never claimed before.
    """
    user = usersdb.find_one({"id": user_id}, {"last_claim": 1, "_id": 0})
    if user and "last_claim" in user:
        return float(user["last_claim"])
    return None

async def update_last_claim_time(user_id: str, timestamp: float):
    """
    Update or create the last claim timestamp for the user.
    """
    usersdb.update_one({"id": user_id}, {"$set": {"last_claim": timestamp}}, upsert=True)
